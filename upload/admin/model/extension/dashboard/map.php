...
class ModelExtensionDashboardMap extends Model {
	...
	public function getRegionsByOrders() {
		...
		if ($implode) {
			$query = $this->db->query("SELECT z.code AS regcode, os.color FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "zone` z ON (o.payment_zone_id = z.zone_id) LEFT JOIN `" . DB_PREFIX . "order_status` os ON (o.order_status_id = os.order_status_id) WHERE o.order_status_id IN(" . implode(',', $implode) . ") GROUP BY o.payment_zone_id");
			return $query->rows;
		} else {
			return array();
		}
